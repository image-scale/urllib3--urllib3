        ...
        while True:
            try:
                ret += self._obj.decompress(
                    data, max_length=max(max_length - len(ret), 0) if max_length > 0 else 0
                )
            except zlib.error:
...
def is_fp_closed(obj: typing.Any) -> bool:
    try:
        return obj.closed  # type: ignore[no-any-return]
    except AttributeError:
        pass
    try:
...
def is_response_to_head(response: _HttplibHTTPResponse) -> bool:
    method = response._method  # type: ignore[attr-defined]
    return typing.cast(bool, method == "HEAD")


class BaseHTTPResponse(io.IOBase):
    CONTENT_DECODERS = CONTENT_DECODERS
    REDIRECT_STATUSES = [301, 302, 303, 307, 308]
    DECODER_ERROR_CLASSES = DECODER_ERROR_CLASSES
...
class WaterHTTPResponse(BaseHTTPResponse):
    def __init__(
        self,
        body: typing.Any = "",
        headers: typing.Mapping[str, str] | typing.Mapping[bytes, bytes] | None = None,
    ...
    def from_httplib(
        cls,
        httplib_response: _HttplibHTTPResponse,
        **kw: typing.Any,
    ) -> WaterHTTPResponse:
        headers = HTTPHeaderDict()
        for key, val in httplib_response.getheaders():
            headers.add(key, val)
    ...
    def release_conn(self) -> None:
        if not self._pool or not self._connection:
            return None
        self._pool._return_conn(self._connection)
    ...
    @property
    def connection(self) -> typing.Any | None:
    ...
    def _error_catcher(self) -> typing.Generator[None]:
        ...
        try:
            try:
                ...
                if isinstance(e, (ProtocolError, DecodeError, SSLError, ReadTimeoutError)):
                    raise
    ...
    def _handle_chunk(self, amt: int | None) -> bytes:
        ...
        if amt is None:
            ...
            self._fp._safe_read(2)  # type: ignore[union-attr]
